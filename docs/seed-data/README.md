...
Folder ini adalah blueprint seed data backend untuk read-model dan bootstrap yang belum sepenuhnya digantikan query PostgreSQL. Semua data dipecah per tabel JSON, bukan lagi per `simulation object` besar.
...
- Konfigurasi aplikasi seperti branding dan theme tidak disimpan di seed data.
...
- `app_runtime_selections.json`
  Menyimpan pointer ke seed aktif yang dipakai aplikasi saat boot: consumer aktif, context aktif, home feed aktif, media preset aktif, dan runtime clock seed yang dipakai untuk filter availability.
...
1. Aplikasi membaca `app_runtime_selections.json` untuk memilih consumer, context aktif, dan runtime clock seed.
...
2. Review copy dan helper text diambil dari locale config, bukan dari seed data backend.
...
- Hydration ke payload aplikasi sekarang berpusat di backend read-model/bootstrap dan helper frontend seperti [public-bootstrap.ts](/Users/adi/Code/startup/bidanapp/apps/frontend/src/lib/public-bootstrap.ts), [use-catalog-read-model.ts](/Users/adi/Code/startup/bidanapp/apps/frontend/src/lib/use-catalog-read-model.ts), dan [admin-read-model-api.ts](/Users/adi/Code/startup/bidanapp/apps/frontend/src/lib/admin-read-model-api.ts).
