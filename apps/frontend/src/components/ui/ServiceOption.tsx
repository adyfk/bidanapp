...
export const ServiceOption = ({ title, price, active, onClick }: ServiceOptionProps) => (
  ...
  >
    <span className={`text-[12px] font-medium ${active ? '' : 'text-gray-500'}`} style={{ color: active ? APP_CONFIG.colors.primary : undefined }}>{title}</span>
    <span className={`text-[15px] font-bold mt-1`} style={{ color: active ? APP_CONFIG.colors.primary : APP_CONFIG.colors.textMain }}>
      {price}
    </span>
  </button>
);
