...
import { CalendarDays, Clock3, House, Search, Stethoscope, Video } from 'lucide-react';
...
export const ProfessionalServicesSection = ({
  ...
  return (
    <section className="rounded-[28px] bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 space-y-3">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            <button
              ...
              className={`whitespace-nowrap rounded-full px-3 py-2 text-[12px] font-semibold transition-colors ${
                activeModeFilter === 'all' ? 'text-white' : 'bg-white text-gray-600'
              }`}
              style={{
                backgroundColor: activeModeFilter === 'all' ? APP_CONFIG.colors.primary : '#FFFFFF',
                boxShadow:
                  activeModeFilter === 'all'
                    ? '0 12px 28px -20px rgba(233, 30, 140, 0.42)'
                    : 'inset 0 0 0 1px rgba(229, 231, 235, 1)',
            ...
            {(['online', 'home_visit', 'onsite'] as ServiceDeliveryMode[]).map((mode) => (
              <button
                type="button"
                key={mode}
                onClick={() => setActiveModeFilter(mode)}
                className={`whitespace-nowrap rounded-full px-3 py-2 text-[12px] font-semibold transition-colors ${
                  activeModeFilter === mode ? 'text-white' : 'bg-white text-gray-600'
                }`}
                style={{
                  backgroundColor: activeModeFilter === mode ? APP_CONFIG.colors.primary : '#FFFFFF',
                  boxShadow:
      ...
      {selectedServiceEntry ? (
        <div className="mt-5 rounded-[26px] border border-transparent bg-[linear-gradient(180deg,#FFFFFF_0%,#FFF7FB_100%)] p-5 shadow-[0_18px_40px_-30px_rgba(17,24,39,0.35)]">
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2">
                <h4 className="text-[19px] font-bold text-gray-900">{selectedServiceEntry.catalogService.name}</h4>
                <span
          ...
          <div className="mt-5">
            <p className="text-[12px] font-semibold uppercase tracking-[0.16em] text-gray-500">
              {t('chooseServiceMode')}
            </p>
                ...
                return (
                  <button
                    ...
                    className={`rounded-[20px] border px-4 py-3 text-left transition-all ${
                      isSelectedMode ? 'shadow-[0_16px_32px_-24px_rgba(233,30,140,0.5)]' : 'shadow-none'
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="text-[13px] font-bold text-gray-900">{getModeLabel(t, mode)}</p>
          ...
          {requiresOfflineScheduleSelection ? (
            <div className="mt-5 space-y-4 rounded-[22px] bg-white/70 p-4 shadow-[inset_0_0_0_1px_rgba(243,244,246,1)]">
              <div>
                <p className="flex items-center gap-2 text-[12px] font-semibold uppercase tracking-[0.16em] text-gray-500">
