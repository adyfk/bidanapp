...
import { SIMULATION_MESSAGES, fillSimulationTemplate } from '@/lib/constants';
    ...
    <div className="fixed inset-y-0 left-1/2 z-[70] flex w-full max-w-md -translate-x-1/2 flex-col bg-white animate-in fade-in slide-in-from-bottom-4 duration-300">
      ...
      <div className="custom-scrollbar flex-1 overflow-y-auto p-5">
        ...
        <div className="mb-8 flex justify-center gap-2">
          {[1, 2, 3, 4, 5].map((star) => (
            <button key={star} type="button" onClick={() => onUpdateRating(star)} className="p-1 transition-transform active:scale-95">
        ...
        <div className="mb-6">
          <label className="mb-2 block text-[13px] font-bold text-gray-700">{SIMULATION_MESSAGES.review.photoLabel}</label>
          <label
            htmlFor={inputId}
        ...
        <div className="mb-6">
          <label className="mb-2 block text-[13px] font-bold text-gray-700">{SIMULATION_MESSAGES.review.reviewLabel}</label>
          <textarea
            value={reviewText}
